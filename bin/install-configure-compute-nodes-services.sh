...
./pssh-exe H "yum install -y centos-release-openstack-mitaka"
./pssh-exe H "yum install -y python-openstackclient openstack-selinux openstack-utils"
### 1. OpenStack Compute service
./pssh-exe H "yum install -y openstack-nova-compute"
./pssh-exe H "yum install -y openstack-neutron-openvswitch"
./pssh-exe H "yum install -y openstack-neutron openstack-neutron-ml2 openstack-neutron-openvswitch"
./pssh-exe H "yum install -y ceph-common"
./pssh-exe H "yum install -y openstack-ceilometer-compute python-ceilometerclient python-pecan"
for ((i=0; i<${#hypervisor_map[@]}; i+=1));
do
  name=${hypervisor_name[$i]};
  ip=${hypervisor_map[$name]};
  data_nic_ip=$(ssh root@${ip} ip addr show dev $data_nic scope global | grep "inet " | sed -e 's#.*inet ##g' -e 's#/.*##g')
  ssh root@@ip /bin/bash << EOF
    ...
    openstack-config --set /etc/neutron/plugins/ml2/openvswitch_agent.ini ovs local_ip $data_nic_ip
...
./pssh-exe H "systemctl enable libvirtd.service openstack-nova-compute.service"
./pssh-exe H "systemctl start libvirtd.service openstack-nova-compute.service"
./pssh-exe H "systemctl enable openstack-nova-compute.service"
./pssh-exe H "systemctl enable openstack-ceilometer-compute.service"
./pssh-exe H "systemctl start openstack-ceilometer-compute.service"
./pssh-exe H "systemctl restart openstack-nova-compute.service"
./pssh-exe H "systemctl start openvswitch.service"
./pssh-exe H "systemctl restart neutron-openvswitch-agent.service"
./pssh-exe H "systemctl enable neutron-openvswitch-agent.service"
