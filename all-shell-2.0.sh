...
data_network=10.10.10.
store_network=11.11.11.
...
for ((i=0; i<${#nodes_map[@]}; i+=1));
  do
      ...
      ping -c 2 $ip
      echo ">>>>>>>"
      ping -c 2 $data_network$(echo $ip|awk -F "." '{print $4}')
      echo ">>>>>>>"
      ping -c 2 $store_network$(echo $ip|awk -F "." '{print $4}')
  done;
 #############################################
 ########       设置SSH          #############
 #############################################
...
for ((i=0; i<${#nodes_map[@]}; i+=1));
  do
      ...
      ssh root@$ip echo `chronyc sources`
  done;
...
for ((i=0; i<${#controller_map[@]}; i+=1));
  do
        ...
        ssh root@$ip rabbitmqctl add_user openstack 123456;
...
### [任一节点]创建数据库
mysql -uroot -p123456 -h $virtual_ip -e "CREATE DATABASE keystone;
...
for ((i=0; i<${#controller_map[@]}; i+=1));
  do
        ...
        ssh root@$ip chmod -R +x $target_sh
        ssh root@$ip $target_sh/$sh_name_2 $virtual_ip
  done;
/root/keystonerc_admin
openstack endpoint list
openstack service list
...
openstack-config --set /etc/glance/glance-api.conf database connection mysql+pymysql://glance:123456@$vip/glance
...
openstack-config --set /etc/glance/glance-registry.conf database connection mysql+pymysql://glance:123456@$vip/glance
...
### [任一节点]创建数据库
mysql -uroot -p123456 -h $virtual_ip -e "CREATE DATABASE glance;
...
openstack-config --set /etc/nova/nova.conf api_database connection mysql+pymysql://nova:123456@$vip/nova_api
openstack-config --set /etc/nova/nova.conf database connection mysql+pymysql://nova:123456@$vip/nova
...
### [任一节点]创建数据库
mysql -uroot -p123456 -h $virtual_ip -e "CREATE DATABASE nova;
...
openstack-config --set /etc/neutron/neutron.conf database connection mysql+pymysql://neutron:123456@$vip/neutron
...
for ((i=0; i<${#controllers_map[@]}; i+=1));
 ...
 ##******************************************************************##
 ### 存储节点上执行 集群  install-openstack-ceph-backend.sh
...
sudo virsh secret-set-value --secret 032198f4-b815-4254-9de2-185f935bd7de --base64 $(cat /etc/ceph/ceph.client.cinder.keyring |grep 'key ='|awk '{print $3}') && rm client.cinder.key secret.xml
...
### [所有控制节点] 修改配置文件
openstack-config --set /etc/ceilometer/ceilometer.conf database connection mongodb://ceilometer:123456@controller01:27017,controller02:27017,controller03:27017/ceilometer?replicaSet=ceilometer
...
openstack-config --set /etc/ceilometer/ceilometer.conf oslo_messaging_rabbit rabbit_password 123456
...
openstack-config --set /etc/ceilometer/ceilometer.conf keystone_authtoken password 123456
...
openstack-config --set /etc/ceilometer/ceilometer.conf service_credentials password 123456
...
openstack-config --set /etc/ceilometer/ceilometer.conf publisher telemetry_secret 123456
...
virtual_ip=192.168.2.201
virtual_ip_redis=192.168.2.202
local_bridge='br-ex'
...
### [controller01] 设置副本集，等待所有节点启动mongod服务后
finish_flag=0
while_flag=0
service="mongod"
...
for ((i=0; i<${#controller_map[@]}; i+=1));
  do
        ...
        scp $source_sh_1 root@$ip:$target_sh
        ssh root@$ip chmod -R +x $target_sh
        ssh root@$ip $target_sh/$sh_name_1 $virtual_ip $virtual_ip_redis $local_bridge
  done;
...
### [所有控制节点] 修改配置文件
openstack-config --set /etc/aodh/aodh.conf database connection mysql+pymysql://aodh:123456@$vip/aodh
...
openstack-config --set /etc/aodh/aodh.conf oslo_messaging_rabbit rabbit_password 123456
...
openstack-config --set /etc/aodh/aodh.conf keystone_authtoken password 123456
...
openstack-config --set /etc/aodh/aodh.conf service_credentials password 123456
...
### [任一节点]创建数据库
mysql -uroot -p123456 -h $virtual_ip -e "CREATE DATABASE aodh;
...
. /root/keystonerc_admin
openstack user create --domain default --password 123456 aodh
...
##******************************************************************##
#### 每个计算节点上配置脚本 sh/compute_nodes_ceilometer_exec.sh
#!/bin/sh
vip=$1
...
##******************************************************************##
#### scp到其他计算节点执行 install-compute-nodes-ceilometer.sh
#!/bin/sh
...
sh_name=compute_nodes_ceilometer_exec.sh
source_sh=$(echo `pwd`)/sh/$sh_name
target_sh=/root/tools/t_sh/
